...
{
    // LVGL keeps its draw buffers in PSRAM on this board.  The SPI driver must
    // use a small internal-DMA staging buffer for each queued color transfer,
    // so do not let the QSPI device silently fan out ten large transfers at
    // once.  The BSP Kconfig default is two and is the safe bound for the
    // ESP32-S3's internal DMA heap.
    const int max_transfer_size = (config && config->max_transfer_sz > 0)
...
static lv_display_t *bsp_display_lcd_init(const bsp_display_cfg_t *cfg)
{
    ...
    esp_lv_adapter_display_config_t disp_cfg = {
        ...
        .profile = {
            ...
            // Keep the active LVGL stripe in internal DMA-capable RAM.  A
            // PSRAM stripe forces the SPI driver to allocate a temporary
            // internal staging buffer for every queued chunk; large overlays
            // such as the Connection Center can otherwise exhaust that heap
            // and leave white tiles on the ST77916.
            .buffer_height = 30,
            .use_psram = false,
            .enable_ppa_accel = false,
            // One direct-DMA stripe is sufficient at 12fps and makes redraws
            // deterministic under BLE/Wi-Fi/audio load.
